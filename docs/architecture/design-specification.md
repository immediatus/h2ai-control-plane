...
| Safety | Rules / guardrails | Guardrails | ADR Auditor gate + J_eff gate + Multiplication check |
...
| Tool risk | Implicit, untracked | Function signatures | `AgentTool` flags → c_i, α, κ_base, NKey scope |
...
| **Server** | Dedicated server/VM | StaticProvider | NatsKvStore | BrokerPublisher + RedactionMiddleware |
...
- **κ_base (pairwise coherency cost):** WebSearch and external API calls introduce retrieval nondeterminism, raising CG variance and therefore κ_eff.
...
The workspace contains 12 crates. The dependency flow is acyclic:

```
api
 └── orchestrator → autonomic · state · context · adapters
...
See [`runtime-phases.md`](runtime-phases.md) for full phase descriptions, the 17-event vocabulary, structural guarantees, and SSE stream lifecycle.
...
| **Memory** | `h2ai-memory` crate; context assembled per task (wire-up planned) | `h2ai-memory` |
...
| **State management** | `TaskStore` + NATS JetStream; event-sourced CRDT | `state`, `h2ai-nats` |
| **Error handling** | 4-class error taxonomy: transient / recoverable / user-fixable / unexpected | `orchestrator::engine` |
| **Guardrails / safety** | ADR Auditor gate; J_eff gate; Multiplication Condition; NKey scoped NATS credentials | `adapters`, `context` |
...
| κ_eff | κ_base / CG_mean | Effective coherency cost per pair |
| N_max | sqrt((1−α) / κ_eff) | Throughput ceiling; adding agents beyond this degrades performance |
