...
- **Does semantic gate coverage improve verification pass rate?** C-004, C-005, C-008 previously scored 0.0 on benchmark tasks. Adding Exclusion + Presence gates (and Ordering for C-005) should increase non-zero verification scores. The counterfactual baseline (bare LlmJudge, no structural gates) vs gated corpus is the experiment. Benchmark scenario: `tests/e2e/scenarios/benchmark`.
