...
- **CSPR-v2 patch repair on retry (2026-05-20, config-gated).** When `cspr.enabled = true`, the `RetryWithHints` branch uses `build_repair_context()` (`h2ai-autonomic/src/repair.rs`) instead of full-regeneration tombstone injection. Repair context anchors the next generation on `global_best_proposal` (the highest-scored proposal seen across all retry waves), emits REPAIR TARGET instructions only for violated constraints, and adds a `[COMPETING CONSTRAINTS DETECTED]` MetaRepair block when the `ConstraintConflictGraph` (`h2ai-constraints/src/conflict.rs`) detects that two violated constraints are structurally incompatible — preventing the LLM from receiving contradictory repair directives. The FALCON Bounded-Locality Theorem (`fp(ℛ(x̂)) ≤ fp(x̂) + Lf·α·v(x̂)`) justifies patch repair over full regeneration: quality loss is proportional to violation magnitude when `α ≪ 1`. Default: `cspr.enabled = false` — all existing behaviour unchanged.
