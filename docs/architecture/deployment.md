...
- Dark Knowledge Compiler enforces typed constraints — the constraint corpus is mounted as a shared volume, not a local directory
- Prometheus metrics scraped by a team-facing Grafana instance
- **Agent provider:** `NatsAgentProvider` (recommended) — maintains a live registry of managed agents that self-register via NATS heartbeats. No container orchestration overhead; long-lived agents dispatch without spawning new containers. Alternatively: `StaticProvider` with containers managed via `docker compose` or Podman Quadlets.
...
- The 23-event vocabulary — no schema changes across plans
...
├── ConfigMap/constraint-corpus      # team constraint documents (ADRs + typed ConstraintDocs)
...
- `h2ai_zero_survival_total` rate increasing — Auditor rejection rate high, check constraint corpus coverage
...
| `H2AI_CONSTRAINT_CORPUS_PATH` | `./adr` | Path to constraint corpus directory (ADRs + typed ConstraintDoc files) |
