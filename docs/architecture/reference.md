...
| `POST` | `/:tenant_id/tasks/:task_id/approve` | **Deprecated** — returns 308 redirect to `/signal`. Kept for one release. |
...
| `POST` | `/:tenant_id/admin/reset-experiment-state` | Reset per-tenant experiment state (bandit arms, topology hint). For controlled experiments only. |
...
| `TieredExit` | `TieredExitEvent` | any (tiered early exit gate fired) |
| `OraclePending` | `OraclePendingEvent` | post-merge (async Phase 6 oracle dispatch) |
| `OracleResult` | `OracleResultEvent` | post-merge (async oracle sidecar result) |
| `CalibrationDrift` | `CalibrationDriftWarning` | any (ECE exceeded 0.15 threshold) |
| `OracleSuspect` | `OracleSuspectEvent` | any (oracle pass rate outside healthy range) |
| `ConstraintAmbiguityDetected` | `ConstraintAmbiguityDetectedEvent` | 3.5 (accumulated ambiguity evidence crossed threshold, GAP-F8/F9) |
...
struct CalibrationCompletedEvent {
    ...
    calibration_quality: CalibrationQuality,        // Domain | Bootstrap
    calibration_source: CalibrationSource,          // Measured | PartialFit | SyntheticPriors
    beta_quality: Option<f64>,                      // conflict-rate β from Phase B; None when < 2 proposals
}
```
...
Emitted once per TAO agent turn, recording the turn observation and pass/fail status.
...
struct TaskAttributionEvent {
    ...
    tokens_used: u64,                               // total tokens across all adapter calls
    skill_nodes_injected: u32,                      // Synthetic skill nodes in thinking loop context (0 = cold start)
...
struct PendingApprovalEvent {
    ...
    q_confidence: f64,
    prediction_basis: u8,            // 0=Heuristic 1=Bootstrap 2=Conformal
    n_used: u32,
    risk_level: ApprovalRiskLevel,   // Low | Medium | High
...
Emitted by `POST /{tenant_id}/tasks/{id}/signal` (via HITL flow) after the approval decision is processed.
...
The `/metrics` endpoint exposes the counters and gauges defined in `crates/h2ai-api/src/metrics.rs`:
...
| `h2ai_phase15_task_quadrant_total{quadrant="precision\|coverage\|complex\|degenerate"}` | counter | Phase 1.5 task routing quadrant distribution. |
| `h2ai_oracle_ece_gauge` | gauge | Current ECE (Expected Calibration Error). Target < 0.05, warn > 0.15. |
...
| `h2ai_oracle_residual_p90` | gauge | P90 of calibration residuals (proxy for conformal interval width). |
| `h2ai_calibration_basis` | gauge | Active `PredictionBasis`: 0=Heuristic, 1=Bootstrap, 2=Conformal. |
...
| `h2ai_calibration_source{source="measured\|partial_fit\|synthetic_priors"}` | gauge | Active calibration source (1 = this variant is active). |
| `h2ai_shadow_audit_total` | counter | Total Phase 4 shadow auditor observations. |
| `h2ai_shadow_audit_disagreements_total` | counter | Shadow audit observations where primary and shadow disagreed. |
| `h2ai_shadow_audit_promoted_domains` | gauge | Domains currently in two-auditor AND-vote mode. |
| `h2ai_shadow_audit_disagreement_rate` | gauge | Rolling disagreement rate across all domains (0–1). |
| `h2ai_safety_profile{profile="..."}` | gauge | Active safety profile (1 = this profile is active). |
| `h2ai_safety_krum_fault_tolerance` | gauge | Krum fault tolerance setting (`f`). |
...
| `h2ai_safety_require_bivariate_cg` | gauge | Bivariate CG check required (1=yes, 0=no). |

`h2ai_n_eff_prior` is updated on every `CalibrationCompletedEvent`. The MAPE-K counters are updated on every task that reaches a MAPE-K decision or successful merge.
...
| `krum_threshold` | `0.30` | `max(c_i)` above which Krum is preferred over `ConsensusMedian` (only when `krum_fault_tolerance > 0`). |
...
| `strict` | `false` (dev) / `true` (prod/strict) | When `true`, shadow vote is binding (AND with primary) on every proposal; when `false`, used for domain-promotion tracking only |
| `promotion_threshold` | `0.05` | Rolling-window disagreement rate above which majority-vote activates for a domain |
| `promotion_window` | `30` | Observations per domain required before promotion is considered |
| `auto_demotion` | `true` | Remove domain from majority-vote set when its disagreement rate drops below `promotion_threshold / 2` |
...
| `calibration_max_tokens` | `32768` | Per-probe token budget. |
...
system_modifier   = "compress_reasoning" # CompressReasoning | FullTask | ConstraintSkeleton
probe_task_source = "same"               # same | synthetic
...
| `calibration_probe.system_modifier` | `"compress_reasoning"` | System context modifier: `CompressReasoning` (compressed skeleton), `FullTask` (unmodified prompt), or `ConstraintSkeleton` (compiled from constraint YAML). |
| `calibration_probe.probe_task_source` | `"same"` | `same` — use production prompt truncated to budget; `synthetic` — compile canonical task from constraint YAML for stationary k-regression. |
...
`ConvergenceGateEvent` is emitted when the gate fires, carrying `convergence_fraction` (mean pairwise cosine), `n_live` (surviving count), `wave`, `theta_converge`, and `best_score`. The `mean_pairwise_cosine()` function computes the upper-triangle mean cosine from surviving proposal embeddings.
...
| `synthesis_critique_max_tokens` | `32768` | Critique stage budget. |
| `synthesis_max_tokens` | `32768` | Synthesis stage budget. |
...
**Spec boundary construction:** The `effective_spec` passed to `GroundingChecker::new()` is assembled from two sources: `manifest.description` and the constraint corpus, where each `ConstraintDoc` contributes `"{c.id}: {c.description}"` joined by newlines. `manifest.context` is intentionally excluded from `effective_spec` — it is routed separately to `TaskContextSeeder::seed_uncertainty_gaps()` which seeds `UncertainDomain` gaps from uncertainty keywords. `ConstraintDoc.binary_checks` and `ConstraintDoc.pass_criteria` are also excluded. Only technologies named in `manifest.description` or a constraint's `description` field are treated as grounded and will not produce `UngroundedContent` gaps.
...
| `tao_per_turn_timeout_secs` | `600` | Per-turn adapter call timeout in seconds. Increase for slow local models. Cloud models typically need 30–120s; queued local models may need the full 600s. |
...
| Field | Default | Purpose |
|---|---|---|
| `explorer_max_tokens` | `32768` | Per-explorer generation budget. |
| `max_context_tokens` | `8192` | Cap kept after context compaction. |
...
| `hitl.timeout_ms` | `1_800_000` | Base review window (30 minutes). Decays exponentially on consecutive non-responses (see `timeout_decay`). |
...
`[constraint_wiki]` is a tagged-enum field (`mode` is the discriminator). When absent from the config, defaults to `Disabled`.

```toml
# Disabled (default) — no constraint resolution
[constraint_wiki]
mode = "disabled"

# FS-backed — load from local YAML corpus
[constraint_wiki]
mode = "fs"
corpus_path = "/constraints"
resolve_k = 50   # reserved: max constraints per resolve_context call
```

| Field | Default | Purpose |
|---|---|---|
| `constraint_wiki.mode` | `"disabled"` | `"disabled"` — no constraint resolution (default). `"fs"` — load from local YAML corpus at `corpus_path`. |
| `constraint_wiki.corpus_path` | — | (fs mode only) Filesystem path for `YamlDirSource`. |
| `constraint_wiki.resolve_k` | `50` | (fs mode only) Reserved: max constraints returned per `resolve_context` call. |
