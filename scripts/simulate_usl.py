...
def topology_gain(c_i: float, n: int, alpha: float, kappa_e: float) -> float:
    """Definition 13 G_topology component.

    Uses actual USL throughput X(N) = N / (1 + α(N-1) + κN(N-1)).
...
def harness_attribution(c_i: float, n: int, alpha: float, kappa_e: float,
                         filter_ratio: float, tao_turns: float) -> dict:
    """Definition 13: full Q_total decomposition."""
    baseline = 1.0 - c_i
    g_topo   = topology_gain(c_i, n, alpha, kappa_e)
...
ALPHA_AI   = 0.15
KAPPA_AI   = 0.025   # kappa_eff for AI agents (κ_base/CG_mean = 0.01/0.4)
...
for component in ["baseline", "topology", "tao", "verification"]:
    ...
    for _, n, turns, fr in CONFIGS:
        attr = harness_attribution(C_I_DEFAULT, n, ALPHA_AI, KAPPA_AI, fr, turns)
...
for component in ["baseline", "topology", "tao", "verification"]:
    values = []
    for _, n, turns, fr in CONFIGS:
        attr = harness_attribution(C_I_SHELL, n, ALPHA_AI, KAPPA_AI, fr, turns)
        values.append(attr[component])
...
C_I_BASE   = 0.50
N_MAX_AI   = int(n_max(ALPHA_AI, KAPPA_AI))   # ≈ 6

# Panel A: Q_total vs N (1..N_max) — topology gain
ax = axes[0]
ns = np.arange(1, N_MAX_AI + 1)
q_vs_n = [harness_attribution(C_I_BASE, int(n_v), ALPHA_AI, KAPPA_AI,
                               filter_ratio=1.0, tao_turns=1)["total"]
          for n_v in ns]
ax.plot(ns, q_vs_n, color="#2563eb", linewidth=2.5, marker="o", markersize=8)
ax.fill_between(ns, harness_attribution(C_I_BASE, 1, ALPHA_AI, KAPPA_AI, 1.0, 1)["total"],
...
q_vs_tao = [harness_attribution(C_I_BASE, 4, ALPHA_AI, KAPPA_AI,
...
q_vs_fr  = [harness_attribution(C_I_BASE, 4, ALPHA_AI, KAPPA_AI,
...
q_fr_min = harness_attribution(C_I_BASE, 4, ALPHA_AI, KAPPA_AI, 1.0, 1.0)["total"]
q_fr_max = harness_attribution(C_I_BASE, 4, ALPHA_AI, KAPPA_AI, 0.0, 1.0)["total"]
...
print(f"\n  Harness attribution — Executor (c_i={C_I_DEFAULT}, AI layer α={ALPHA_AI}, κ_eff={KAPPA_AI}):")
