...
use h2ai_orchestrator::grounding_chain::{
    format_grounding_hint, gap_hint_or_query, gap_queries_from_record, synthesis_meets_threshold,
    GapResearchChain, GroundingContext, GroundingProvider, GroundingResult, GroundingSource,
    LlmResearcherGrounder, SpecAnchorGrounder, WebSearchGrounder,
};
...
/// Spec-anchor-only chain: grounding events carry SpecAnchor source.
#[tokio::test]
    ...
    for ev in &output.researcher_grounding_events {
        if !ev.shared_assumption.is_empty() {
            assert_eq!(
                ...
                "grounding event must carry SpecAnchor source"
            );
            assert!(
                ev.slot.is_some(),
...
/// Tests GapResearchChain::resolve directly.
#[tokio::test]
async fn system_web_search_chain_escalates_correctly_per_tier() {
    ...
    let chain = GapResearchChain::new(vec![
...
// ── truncate_at_sentence via GapResearchChain::resolve distillation ────────
...
async fn chain_distillation_replaces_raw_web_text_with_distilled_output() {
    ...
    let chain = GapResearchChain::new(providers).with_distiller(distiller, true, 256);
...
async fn chain_distill_disabled_preserves_raw_text_capped_at_hint_limit() {
    ...
    let chain = GapResearchChain::new(providers).with_distiller(distiller, false, 256);
    let result = chain.resolve(&ctx, 1).await.unwrap();
    // distill=false: raw text passes through unchanged (no truncation in this project)
    assert!(!result.grounding_statement.is_empty());
}

// ── GapResearchChain unit tests ────────────────────────────────────────────
...
// Test removed: tested the correlated-fabrication phase which has been deleted from the pipeline.

#[cfg(test)]
mod classify_slot_tests {
    use h2ai_orchestrator::grounding_chain::classify_grounding_slot;
...
fn chain_len_and_is_empty() {
    let chain_empty: GapResearchChain = GapResearchChain::new(vec![]);
    assert_eq!(chain_empty.len(), 0);
    assert!(chain_empty.is_empty());

    let chain_one = GapResearchChain::new(vec![Box::new(SpecAnchorGrounder)]);
...
async fn chain_empty_providers_returns_none() {
    let chain: GapResearchChain = GapResearchChain::new(vec![]);
    let ctx = GroundingContext {
        fabricated_entities: vec!["CockroachDB".into()],
...
fn synthesis_meets_threshold_works_with_optional_grounding_chain() {
    ...
    // run_gap_researcher now accepts Option<&GapResearchChain> instead of
    ...
    assert_eq!(queries.len(), 3);
    // Verify that passing None for the chain (type-checked as Option<&GapResearchChain>)
    // is expressible in user code.
    let chain: Option<&GapResearchChain> = None;
