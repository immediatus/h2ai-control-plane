...
use h2ai_types::agent::{AgentDescriptor, AgentTelemetryEvent, AgentTool, TaskPayload, TaskResult};
use h2ai_types::identity::AgentId;
use h2ai_types::physics::TauValue;
use std::time::Duration;

async fn spawn_fake_agent(nats: Client, agent_id: AgentId) {
...
async fn execute_and_await_returns_task_result() {
    ...
    let agent_id = AgentId::from("test-agent-1");
    spawn_fake_agent(nats, agent_id).await;
