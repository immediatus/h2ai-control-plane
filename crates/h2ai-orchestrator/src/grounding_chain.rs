...
use h2ai_config::prompts::{
    GROUNDING_DISTILL_SYSTEM, GROUNDING_DISTILL_TASK, GROUNDING_RESEARCHER_SYSTEM,
    GROUNDING_RESEARCHER_TASK, I1_SYNTHESIS_VALIDATOR_TASK,
};
...
use crate::gap_checkers::grounding::extract_arch_nouns;
...
impl GroundingProvider for LlmResearcherGrounder {
    async fn ground(&self, ctx: &GroundingContext) -> Option<GroundingResult> {
        ...
        let research_req = ComputeRequest {
            system_context: GROUNDING_RESEARCHER_SYSTEM.as_str().into(),
            task: GROUNDING_RESEARCHER_TASK.render(&[
...
async fn distill_with_llm(
    ...
    let req = ComputeRequest {
        system_context: GROUNDING_DISTILL_SYSTEM.as_str().into(),
        task: GROUNDING_DISTILL_TASK.render(&[
            ("task_description", &ctx.task_description),
            ("raw_results", raw),
        ]),
...
///
/// Builds a `GroundingContext` from the gap record, runs the full `GapResearchChain`
...
pub async fn run_gap_researcher(
    ...
    // the constraint requirement — enough for the grounding injector to correct the explorer.
...
/// Classifies fabricated entities detected by grounding into a repair context slot name.
...
/// by `GapResearchChain::resolve`; this function only strips residual URLs.
#[must_use]
