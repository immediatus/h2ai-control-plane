...
use h2ai_nats::subjects::{ephemeral_task_subject, task_result_subject};
    ...
    A: AuditProvider,
{
    ...
    /// Execute the full task pipeline for one request.
    ///
    /// 1. Fetch recent history for context assembly
    /// 2. Construct TaskPayload
    /// 3. Ensure agent capacity
    /// 4. Publish task to NATS
    /// 5. Record task dispatch as telemetry
    ...
    pub async fn execute_and_await(
        ...
        let mut telemetry_sub = self
            .nats
            .subscribe(format!("h2ai.telemetry.{task_id}"))
            .await
            .map_err(|e| OrchestratorError::Transport(e.to_string()))?;
                ...
                msg = telemetry_sub.next() => {
                    if let Some(msg) = msg {
                        if let Ok(event) = serde_json::from_slice::<AgentTelemetryEvent>(&msg.payload) {
                            let _ = self.record_telemetry(event).await;
                        }
                    }
                }
