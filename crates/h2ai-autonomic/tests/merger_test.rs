...
async fn merge_engine_resolves_crdt_when_valid_proposals_exist() {
    ...
    let outcome = MergeEngine::resolve(task_id, set, vec![], MergeStrategy::ScoreOrdered, 0, None).await;
...
async fn merge_engine_emits_zero_survival_when_all_pruned() {
    ...
    let outcome = MergeEngine::resolve(task_id, set, pruned_events, MergeStrategy::ScoreOrdered, 0, None).await;
...
async fn merge_engine_consensus_median_selects_a_proposal() {
    ...
    let outcome = MergeEngine::resolve(task_id, set, vec![], MergeStrategy::ConsensusMedian, 0, None).await;
...
async fn merge_engine_krum_selects_honest_proposal() {
    ...
    let outcome = MergeEngine::resolve(task_id, set, vec![], MergeStrategy::Krum { f: 1 }, 0, None).await;
    if let MergeOutcome::Resolved { resolved, .. } = outcome {
        assert_ne!(
