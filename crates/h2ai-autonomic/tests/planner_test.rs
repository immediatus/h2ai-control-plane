...
fn auditor() -> AuditorConfig {
    AuditorConfig { adapter: adapter(), ..Default::default() }
}
...
#[test]
fn planner_selects_ensemble_when_weights_balanced() {
    let cc = cc();
    ...
    assert_eq!(event.topology_kind, TopologyKind::Ensemble);
}
...
#[test]
fn planner_computes_positive_n_max_and_kappa_eff() {
    let cc = cc();
    let weights = ParetoWeights::new(0.34, 0.33, 0.33).unwrap();
    let cfg = H2AIConfig::default();
    let event = TopologyPlanner::provision(ProvisionInput {
...
fn planner_selects_bft_consensus_when_evaluator_present() {
    ...
    assert_eq!(event.merge_strategy, MergeStrategy::BftConsensus);
}
