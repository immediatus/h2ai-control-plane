...
/// # Panics
///
/// Panics when `[web_search]` is configured but the required env vars
/// (`api_key_env`, `cx_env`) are missing or empty, or when `[wasm_executor]`
/// is configured but `interpreter_wasm_path` does not exist on disk.
pub fn validate_tool_configs(cfg: &H2AIConfig) {
    ...
    if let Some(wasm) = &cfg.wasm_executor {
        assert!(std::path::Path::new(&wasm.interpreter_wasm_path).exists(),
