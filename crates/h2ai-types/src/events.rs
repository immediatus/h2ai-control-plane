...
pub struct ConstraintViolation {
    ...
    /// Per-check LLM reason strings parsed from LlmJudge CoT, aligned with `check_verdicts`.
    /// `None` when reason parsing is unavailable; `Some([])` when reasons were expected but empty.
    #[serde(default)]
    pub check_reasons: Option<Vec<String>>,
}
...
pub struct BranchPrunedEvent {
    ...
    pub timestamp: DateTime<Utc>,
    /// Wave index (retry_count) at which this branch was pruned.
    /// Annotated by MapeKController::observe; zero for legacy events.
    #[serde(default)]
    pub retry_count: u32,
    /// When set, indicates this branch was pruned because one or more verifiers entered
    /// frozen-bypass mode; contains a brief human-readable explanation.
    #[serde(default)]
...
pub struct CoherenceIncompleteEvent {
    ...
    pub retries: u32,
    pub timestamp: DateTime<Utc>,
    /// Constraint IDs whose verifiers entered frozen-bypass mode during this task run.
    /// Populated only when at least one verifier was bypassed; empty otherwise.
    #[serde(default)]
    pub bypassed_verifier_constraint_ids: Vec<String>,
}
...
/// Emitted once when a constraint's verifier score has been unchanged (frozen) across
/// `gap_resilience.frozen_verifier_wave_threshold` consecutive waves and bypass is triggered.
///
/// Downstream consumers can use this event to annotate pruned branches and
/// `CoherenceIncompleteEvent` with the set of bypassed constraint IDs.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VerifierFrozenEvent {
    /// The constraint whose verifier is considered frozen.
    pub constraint_id: String,
    /// Wave index at which the verifier first appeared frozen.
    pub frozen_since_wave: u32,
    /// Verifier scores from each wave since the freeze began (most-recent last).
    pub per_wave_scores: Vec<f64>,
    /// Representative rejection reason from the frozen verifier (sampled from last wave).
    pub sample_reason: String,
    /// `true` when the bypass gate actually suppressed this verifier; `false` = diagnostic only.
    pub bypassed: bool,
}
...
pub enum H2AIEvent {
    ...
    /// Wraps [`VerifierFrozenEvent`]: a constraint verifier has been frozen and bypass was triggered.
    VerifierFrozen(VerifierFrozenEvent),
