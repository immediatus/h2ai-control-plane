...
impl AdapterKind {
    ...
    pub fn model_lineage_key(&self) -> String {
        match self {
            Self::CloudGeneric { endpoint, model, provider, .. } => {
...
mod tests {
    ...
    fn monoculture_pool_has_one_distinct_lineage_key() {
        ...
        assert_eq!(keys.len(), 1, "monoculture pool must have exactly 1 lineage key");
    }
    ...
    fn diverse_pool_has_multiple_lineage_keys() {
        ...
        let keys: std::collections::HashSet<String> =
            adapters.iter().map(|a| a.model_lineage_key()).collect();
        assert_eq!(keys.len(), 2, "diverse pool must have 2 distinct lineage keys");
    }
}
