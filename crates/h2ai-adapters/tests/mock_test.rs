use h2ai_adapters::mock::{DecompositionMockAdapter, MockAdapter, SequencedMockAdapter};
...
async fn decomposition_mock_adapter_returns_step3_json_for_json_formatter_context() {
    ...
    // Should return the STEP3_MOCK_JSON which contains role_frame fields
    assert!(
        resp.output.contains("role_frame"),
        "expected STEP3_MOCK_JSON output, got: {}",
        resp.output
    );
